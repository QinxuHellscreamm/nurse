<template>
  <div>
    <div id="echarts" :style="{ height: '300px',width:'550px',margin:'0 auto'}" ></div>
  </div>
</template>
<script>
  export default{
    props : ['department_id' , 'years'],

    data(){
      return {
        type:'Z00100',
        /*date: new Date()*/
        start_end_time: [],
        end_time : "",
        graphiData:'',

        restaurants:[],

        strF:'',
        str:'',
        dataForm:[],
      }
    },

    mounted() {



      this.pageAjax()

    },
    methods:{

      pageAjax(){
        let myChart = this.echarts.init(document.getElementById('echarts'))
        myChart.clear();
        //请求数据response
        this.getSer("datacenter/indicator" ,{
          code:'Z00100',
          war_id:this.department_id,
          year:this.years,
          month:''
        }, (res) => {

          this.dataForm = res.data.items
        console.log("5222", this.dataForm)
        myChart.setOption(this.chartInit())
      })

        //myChart.setOption(this.chartInit())
      },

      chartInit(){


        var xarr=[12,1,4,5,6,7,8,9,2,11,2,14]
        var title=["一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"]
        var percentage=[2,3,4,5,6,7,8,9,0,11,12,14]
        var growthRate=[5,5,5,8,5,8,15,25,8,5,5,15]
       /* this.dataForm.forEach((item , i) => {
          xarr.push(item.item_count)
          title.push(item.item_name)
          percentage.push(item.item_count_percentage)
      })*/
        function getTableLine(num) {
          var list = [];
          var bottom = 112;
          var height = 30;
          for (var i = 0; i < num; i++) {
            list.push({
              type: 'line',
              bottom: bottom - i * height,
              right: 20,
              style: {
                fill: '#333',
                stroke:'#dfdfdf'
              },
              shape: {
                x1: 0,
                y1: 0,
                x2: 2000,//下划线偏离
                y2: 0
              }
            });
          }
          return list;
        }
        var lineList = getTableLine(4);//3行
        return {
          title: [{
            text: '\n'+ '2014'+'\n2013'+'\n增长率',
            bottom: 84,
            left: 10,
            textStyle: {
              lineHeight:30,
              fontSize: 13,
              fontWeight: 'normal',
              formatter: function(value) {
                return '{table|' + value + '}';
              },
              rich: {
                table: {
                  align: 'center'
                }
              }
            }
          }],
          tooltip: {
            trigger: 'axis',
          },
          grid: {
            top:30,
            bottom: 150,
            left: 70,
            right: 20
          },

          xAxis: [{
            type: 'category',
            data: title,
            axisTick: {
              show:true,
              length:128    //x轴垂直线长度
            },
            axisLine: {
              onZero:false,  //从最低值开始计算
              lineStyle:{
                color:'#D1D1D1'
              }
            },
            axisLabel: {
              interval:0,
              textStyle: {
                fontWeight: 'normal',
                fontSize: '12',
                color:"#666"
              },
              formatter: function(value, index) {

                return '{table|' + value +
                  '}\n{table|' + (xarr[index]+"%") +
                  '}\n{table|' + (percentage[index]+"%") +
                  '}\n{table|' + (growthRate[index]+"%") +
                  '}';

              },
              rich: {
                table: {
                  lineHeight: 30,
                  align: 'center'
                },
                lineStyle:{
                  color:'#D1D1D1'
                }
              }
            }
          }],
          yAxis: [{
            type: 'value',
            splitNumber:4,
            axisLine: {
              show: true,
              lineStyle:{
                color:'#D1D1D1'
              }
            },
            axisLabel: {
              formatter: '{value}%',
              textStyle: {
                fontWeight: 'normal',
                fontSize: '12',
                color:"#666"
              }
            },
          }],
          series: [{
            type: 'line',
            data: xarr,
            itemStyle: {
              normal:{
                color: ['#00B3DC']
              },
            },
          },{
            type: 'line',
            data: percentage,
            itemStyle: {
              normal:{
                color: ['#00CA53']
              },
            },
          },{
            type: 'line',
            data: growthRate,
            itemStyle: {
              normal:{
                color: ['#bc09e6']
              },
            },
          }],
          graphic: lineList
        };
      },
    },
  }
</script>
<style scoped lang="stylus">

</style>
