<template>
  <div :class= "{'title': this.nodeData.NODE_TYPE == 2 && this.nodeData.is_father}" style='float: left'>
    <div v-if='nodeData.NODE_TYPE==2' class='titlename' >{{this.nodeData.NODE_NAME}}</div><!--标题-->
    ...
    <div class='node-item item1' v-if='nodeData.INPUT_MODE == 1'><!--单行文本-->
      ...
      <el-input placeholder='请输入' v-model='nodeData.VALUE.content' v-if='nodeData.INPUT_TYPE == 2' :maxlength='nodeData.INPUT_LENGTH'></el-input><!--文本-->

      <el-input placeholder='请输入' v-model='nodeData.VALUE.content' v-if='nodeData.INPUT_TYPE == 1' :maxlength='nodeData.INPUT_LENGTH' :min='nodeData.INPUT_RULES.min' :max='nodeData.INPUT_RULES.max'></el-input><!--数字-->

      <el-time-picker v-if='nodeData.INPUT_TYPE == 3' v-model="nodeData.VALUE.content" placeholder="任意时间点"></el-time-picker>
      ...
      <el-date-picker v-model="nodeData.VALUE.content" value-format='yyyy-MM-dd	HH:mm' format='yyyy-MM-dd	HH:mm' type="datetime" v-if='nodeData.INPUT_TYPE == 5' placeholder="请选择" class='dateandtime'></el-date-picker>
    ...
    <div class="node-item" v-if='this.nodeData.INPUT_MODE == 7'>
      ...
      <el-radio-group v-model="nodeData.VALUE.option[0]">
        <el-radio v-for="(item,index) of radios" :label="index + 1 + ''" :key='index'>{{item.title}}</el-radio>
      </el-radio-group>
      <el-input v-model='nodeData.VALUE.content'></el-input>
    </div>
    <div class="node-item" v-if='this.nodeData.INPUT_MODE == 8'>
      <label class='marg5' style='margin-right: 15px'>
        <span v-if='nodeData.IS_REQUIRED == 1 '>*</span>
        {{ nodeData.NODE_NAME }}
      </label>

      <el-checkbox v-for="(item,index) of radios" :label="index + 1 + ''" :key='index' v-model="nodeData.VALUE.option" @change='checkboxComputedScore'>{{item.title}}</el-checkbox>

      <el-input v-model='nodeData.VALUE.content' style='margin-left: 10px;'></el-input>
    </div>
    ...
    <div v-if='rander' style='overflow: hidden'>
  ...
  export default {
    ...
    created(){
      this.$nextTick(()=>{
        ...
        setTimeout(()=>{
          if(this.nodeData.INPUT_TYPE == '4'||this.nodeData.INPUT_TYPE == '5'){
