<template>
    <el-dialog title="新增授权（将本病室未归档病人授权给其他病室护士，她将可以修改此病人的护理文书）" :visible.sync="dialogVisible" width="800px">
      <div class='body'>
        <el-form label-position="top" :model="from" :inline="true">
          <el-form-item label="病人信息">
            <el-select v-model="from.INPATIENT_NUMBER" filterable placeholder="请选择(或搜姓名)" @change="getPatient">
              <el-option v-for="item in patient_list" :key="item.value" :label="item.NAME" :value="item.INPATIENT_NUMBER" >
                <template slot-scope="{row}"><span>{{item.BED_NUMBER}}床({{item.NAME}})</span></template>
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="被授权科室">
            <el-select v-model="from.DEPARTMENT_ID" filterable placeholder="请选择" @change="getNurse">
  ...
  export default{
    ...
    methods:{
      ...
      getNurse() {
          ...
          this.getSer('datacenter/nurse',{department_id:this.from.DEPARTMENT_ID}, res=>{
    ...
    created(){
      this.$nextTick(() => {
        ...
        this.getSer('datacenter/department',{}, res=>{
          this.inpatientArea = res.data.items;
            this.getSer("datacenter/grantpatient/patientlistforgrant",{ page_size : 1000 , page : 1  }, res => {
