...
<script>
  ...
  export default {
    ...
    props:['INPATIENT_NUMBER','id','re_code'],
    ...
    methods:{
      postMeasures:_debounce(function () {
        ...
        obj2.content = this.num.name + this.content + this.checkList.toString()
        obj2.option = this.option
        obj2.score = 0
        let answers = JSON.stringify([obj,obj2])
        this.post('datacenter/questionnaire',{ from : 1, form_id : this.form_id , inpatient_number : this.INPATIENT_NUMBER ? this.INPATIENT_NUMBER : this.num.INPATIENT_NUMBER , time : _YMDhms() , answers },res=>{
          console.log(res)
          if(res.status<400){
      ...
      getData(data,num,batch){

        this.num = num
        if(!batch) batch=''
        this.getSer('datacenter/twtemplate/issign',{ re_code : this.re_code ? this.re_code : '', tid :this.id ? this.id : '', code:data.VS_CODE ,val : data.VS_VALUE , num : this.INPATIENT_NUMBER ? this.INPATIENT_NUMBER : num.INPATIENT_NUMBER , batch },res=>{
          console.log(res);
