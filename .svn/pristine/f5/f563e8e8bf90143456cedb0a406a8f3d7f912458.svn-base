...
<script>
    ...
    export default {
        ...
        props:['id','INPATIENT_NUMBER','NAME','RE_CODE'],
        ...
        methods:{
          postMeasures:_debounce(function () {
            ...
            let answers = JSON.stringify([obj2])
            this.post('datacenter/questionnaire',{ from : 2, form_id : this.form_id , inpatient_number : this.INPATIENT_NUMBER , time : _YMDhms() , answers },res=>{
              console.log(res)
              if(res.status<400){
          ...
          getData(val){
            ...
            this.getSer('datacenter/twtemplate/issign',{ type:2,tid:this.id ,val : this.computedFraction , num:this.INPATIENT_NUMBER ,re_code:this.RE_CODE},res=>{
              console.log(res);
