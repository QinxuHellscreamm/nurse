...
Vue.prototype.if_session_cookie = function () {

  if(sessionStorage.admin_id && this.getCookieVal('admin_id')){
    if(sessionStorage.admin_id == decodeURIComponent(this.getCookieVal('admin_id'))){

    }else{
      this.$message({type: 'warning', message: '一台设备同时只能登录一个账号'})
      this.$router.push("/login")
    }
