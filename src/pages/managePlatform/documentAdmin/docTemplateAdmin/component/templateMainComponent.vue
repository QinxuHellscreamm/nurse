  ...
  <div>
    ...
    <div class="out-main">
      <el-table :data="LIST" style="width: 100%">
        ...
        <el-table-column prop="NAME" label="模板标题" min-width="200"></el-table-column>
        <el-table-column label="模版编码" min-width="100" prop='TEMPLATE_CODE'></el-table-column>
        <el-table-column label="关联科室" min-width="100" prop='DEPARTMENT_NAME'></el-table-column>
        ...
        <el-table-column  label="操作" min-width="420">
          <template slot-scope="{row}">
            ...
            <el-button  type="text" size="small" @click='preview(row.ID,row.TYPE)'>查看</el-button>
      ...
      <div class="block" style="margin:20px 0;">
        <el-pagination background @current-change="handleCurrentChange" :current-page="currentPage1" :page-size="page_size" layout="total, prev, pager, next, jumper" :total="adicsCount"></el-pagination>
      </div>
    </div>
    <listPreview ref='listPreview'></listPreview>
    <recordPreview ref='recordPreview'></recordPreview>
...
<script>
  ...
  export default {
    ...
    components:{ listPreview, printSet,recordPreview },
    ...
    methods:{
      ...
      preview(id,type){
        if(type == 1) this.$refs.listPreview.getData(id)
        if(type == 2) this.$refs.recordPreview.getData(id)
      }
    },
    mounted() {
      this.$nextTick(() => {
