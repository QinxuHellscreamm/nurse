<template>
    <el-dialog width='1280px' title='血酮单' :visible.sync="popshow">
      <div ref='print'>
        <div ref='label'>
          <h1 style='font-size: 20px;width: 40%; padding-left: 40%;margin-top: 15px' class='hide' >常德市第二人民医院血酮监测表</h1>
          <div>
        ...
        <el-table border :data='list'>
          ...
          <el-table-column label="血酮">
            <el-table-column :label="item.label" v-for='(item,index) of options' :key='item.value'>
              <el-table-column label="数据" prop='BLOOD_KETONE_RECORD' width='46'>
                <template slot-scope="{row}">
                  {{row.DATA[index].BLOOD_KETONE_RECORD}}
                </template>
              </el-table-column>
          ...
          <el-table-column label="随机血酮">
            ...
            <el-table-column label="数据" width='46'>
              <template slot-scope="scope">
                <div v-if='scope.row.DATA[9]&&scope.row.DATA[9][0] && scope.row.DATA[9][0].BLOOD_KETONE_RECORD'>{{ scope.row.DATA[9][0].BLOOD_KETONE_RECORD}}</div>
              </template>
            </el-table-column>
            <el-table-column label="签名" width='76'>
              <template slot-scope="scope">
                <div v-if='scope.row.DATA[9]&&scope.row.DATA[9][0] && scope.row.DATA[9][0].AUTOGRAPHQ'>{{ scope.row.DATA[9][0].AUTOGRAPHQ}}</div>
              </template>
            </el-table-column>
            <el-table-column label="时间" width='170'>
              <template slot-scope="scope">
            ...
            <el-table-column label="数据" width='46'>
              <template slot-scope="scope">
                <div v-if='scope.row.DATA[9]&&scope.row.DATA[9][1] && scope.row.DATA[9][1].BLOOD_KETONE_RECORD'>{{ scope.row.DATA[9][1].BLOOD_KETONE_RECORD}}</div>
              </template>
            </el-table-column>
            <el-table-column label="签名" width='76'>
              <template slot-scope="scope">
    ...
    export default {
        ...
        data(){
          return{
            type:'',
            popshow : false,
            options : [{label:'0:00',value:'1'},{label:'3:00',value:'2'},{label:'早餐前',value:'3'},{label:'早餐后',value:'4'},{label:'午餐前',value:'5'},{label:'午餐后',value:'6'},{label:'晚餐前',value:'7'},{label:'晚餐后',value:'8'},{label:'22:00',value:'9'}],//{label:'随机血酮',value:'10'}
            list : []
          }
        },
