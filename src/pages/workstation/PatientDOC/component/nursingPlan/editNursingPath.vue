            ...
            if(this.title == '编辑护理计划'){
              ...
              if(this.editData.TARGET instanceof Array){
                this.editData.TARGET.forEach(item=>{
                  item.TARGET ? TARGET.push({ TARGET : item.TARGET , ID:item.ID ? item.ID : '' }) : null
                })
              }
              console.log(TARGET)
              TARGET = JSON.stringify([...TARGET , ...this.deletTargetArr])
              ...
              if(this.editData.TARGET instanceof Array){
                this.editData.TARGET.forEach(item=>{
                  item.TARGET ? TARGET.push(item.TARGET) : null
                })
              }
