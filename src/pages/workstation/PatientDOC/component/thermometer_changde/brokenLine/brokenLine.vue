    ...
    <div class='line'>
      <svg style='width: 755px;height:880px'>
        ...
        <template v-for='item of spotArr'>
          ...
          <circle :cx="item.x" :cy="item.y" r="4" stroke="red" fill="white" v-if='item.select_code == 1 && item.type == 1 && item.show'/>
          <text :x="item.x-5" :y="item.y+6" font-size="20" style="fill:blue" v-if='item.select_code == 1 && item.show '><tspan style='font-size: 20px;'>×</tspan></text>

          <text :x="item.x-7" :y="485" font-size="40" style="fill:blue;font-size:30px" v-if='item.select_code == 1 && !item.show '>↓</text>

          <circle :cx="item.x" :cy="item.y" r="4" stroke="red" fill="white" v-if='item.select_code == 2 && item.type == 1 && item.show'/>
          ...
          <circle :cx="item.x" :cy="item.y" r="4" stroke="red" fill="white" v-if='item.select_code == 3 && item.type == 1 && item.show'/>
          <circle :cx="item.x" :cy="item.y" r="4" stroke="blue" fill="blue" v-if='item.select_code == 3 && item.type != 1 && item.show'/>
          <circle :cx="item.x" :cy="item.y" r="3" stroke="blue" fill="blue" v-if='item.select_code == 3 && item.type == 1 && item.show' />

          <circle :cx="item.x" :cy="item.y" r="4" stroke="blue" fill="white" v-if='item.select_code == 4 && item.show'/>
