...
<script>
  import { _debounce } from "@/global"
    export default {
        ...
        props:['editData','tpl','signList','tit1'],
        ...
        methods:{
          ...
          save : _debounce(function () {
            ...
            if (data == '[]' || this.tit.trim() == '') {
              var message
              this.tit.trim() == '' ? message = '标题不能为空' : message = '请填写数据'
                this.$message({ type: 'warning', message: message })
                return false
            }
            this._put('datacenter/vs-nurse-template/'+this.tpl, { id:this.tpl,title : this.tit.trim() , data } , res=>{
              if(res.status < 400){
