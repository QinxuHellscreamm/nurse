<template>
    <div class='temperature'>
      <div class='input-menu'>
        ...
        <el-time-select v-model="time" :picker-options="{start: '2:00',step: '4:00',end: '22:00'}" placeholder="选择时间" class='mar5' @change='getData'></el-time-select>
        <el-button type='primary' @click='$refs.alert.popshow = true' class='filter'>筛选</el-button>
        <el-button type='primary' @click='save'>保存</el-button>
      </div>
      <div class="out-main">
        <el-table :data='tableList' v-if='tableList[0]' ref='focus_el'>
          <el-table-column label="床位信息"  width='96'>
            <template slot-scope="scope">
              {{scope.row.BED_NUMBER +"床 "+ scope.row.NAME}}
            </template>
          </el-table-column>
          <el-table-column :label="parent_template.NAME" v-for='(parent_template,parent_index) of template' :key='parent_template.CODE' width='140'>
            <template slot-scope="scope">
              <div>
                <span v-for='(sub,subindex) in parent_template.SUB_CODE' :key='sub.CODE'>
                  <el-select v-model='scope.row.ITEMS[parent_index][subindex+1].VS_VALUE' placeholder='' v-if='sub.INPUT_TYPE_CODE == "03"' size="mini">
                      <el-option v-for='itemO of sub.OPTION_LIST' :key="itemO.code" :label="itemO.name" :value="itemO.code"></el-option>
                  </el-select>
                  <el-input size="mini" v-model='scope.row.ITEMS[parent_index][subindex+1].VS_VALUE' v-if='sub.INPUT_TYPE_CODE == "01" || sub.INPUT_TYPE_CODE == "02"' @change='type_update(scope.row.ITEMS[parent_index][subindex+1],sub.VALIDATION_RULE,scope.row.INPATIENT_NUMBER)' :maxlength='Number(sub.INPUT_LENGTH)' :data-index='"input_" + parent_index+"_"+subindex' @keyup.native='keyup(scope.$index,"input_" + parent_index+"_"+subindex,$event)'></el-input>
                </span>
                <span>
                  <el-input size="mini" v-model='scope.row.ITEMS[parent_index][0].VS_VALUE' v-if='parent_template.INPUT_TYPE_CODE == "01" || parent_template.INPUT_TYPE_CODE == "02"' :maxlength='Number(parent_template.INPUT_LENGTH)' @change='type_update(scope.row.ITEMS[parent_index][0],parent_template.VALIDATION_RULE,scope.row.INPATIENT_NUMBER)' :class="{'err':scope.row.ITEMS[parent_index][0].err}" :data-index='"input_" + parent_index' @keyup.native='keyup(scope.$index,"input_" + parent_index,$event)'></el-input>
                  <el-autocomplete size="mini" v-if='parent_template.INPUT_TYPE_CODE == "04"' v-model="scope.row.ITEMS[parent_index][0].VS_VALUE" :fetch-suggestions="querySearch_input" placeholder="" @focus='focus(parent_template)' :maxlength='Number(parent_template.INPUT_LENGTH)' @select="type_update(scope.row.ITEMS[parent_index][0],parent_template.VALIDATION_RULE,scope.row.INPATIENT_NUMBER,parent_template.NAME)" @blur='type_update(scope.row.ITEMS[parent_index][0],parent_template.VALIDATION_RULE,scope.row.INPATIENT_NUMBER,parent_template.NAME)' :class="{'err':scope.row.ITEMS[parent_index][0].err}"></el-autocomplete>
                </span>
              </div>
            </template>
          </el-table-column>
  ...
  import signBuildRecord from '@/pages/workstation/component/signBuildRecord'
    export default {
        ...
        methods : {
          ...
          type_update(data,VALIDATION_RULE,INPATIENT_NUMBER,name){
            var obj = {INPATIENT_NUMBER,name}
            data.err = false
            ...
            if(VALIDATION_RULE && VALIDATION_RULE.max && VALIDATION_RULE.min){
              ...
              this.$refs.alertDialog.getData(data,obj,1)
            }
          },
